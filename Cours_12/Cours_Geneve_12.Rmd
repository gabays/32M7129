---
title: "12. Cartographie: aller plus loin"
author: "Simon Gabay"
date: "19/10/2021"
output:
  html_document:
...
boyer  <- read.csv("Data/boyer.csv", fileEncoding="UTF-8")
leaflet(shp)%>%
## On ajoute un fond de carte
addProviderTiles(providers$Esri.NatGeoWorldMap)%>% 
...
#Je peux extraire plusieurs cantons en fonction d'un autre critère que son nom — ici, "huitante" pour "quatre-vingts"
canton_huitante<-Switzerland1_UTM[Switzerland1_UTM@data$eighty == "huitante",]
canton_huitante_df<-fortify(canton_huitante)

#Je peux extraire plusieurs cantons en fonction d'un autre critère que son nom — ici, "quatre-vingts" pour "huitante"
...
```{r}
releves = read.table("Data/80.tsv", header=T, sep="\t", quote="", dec=".", fileEncoding="UTF-8")
head(releves)
```
...
```{r}
georeferecement = read.table("Data/ALF-suisse.tsv", header=T, sep="\t", quote="", dec=".", fileEncoding="UTF-8")
georeferecement
```
